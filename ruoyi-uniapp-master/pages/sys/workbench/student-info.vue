			...
			<u-card class="task-list-item" :border="false" padding="20" margin="20rpx">
				<view slot="head" style="display: flex;align-items: center;justify-content: space-between;">
					<view style="display: flex;align-items: center;font-size: 30rpx;">
						<image class="user-images" src="/static/aidex/images/user03.png"></image>{{item.name}}
					</view>
					<view style="text-align: right;color: green;">
						<view class="apply-text"><span>今日测温：</span>
						<span v-if = "item.tempType === 1">正常</span>
						<span v-else-if = "item.tempType === 2">发热</span>
						<span v-else = "item.tempType === 3">异常</span>
						</view>
					</view>
				</view>
				<view class="" slot="body">
					<u-row gutter="16">
						<u-col span="12">
							<view class="apply-text"><span>性别：</span>
							<span v-if = "item.sex === 0">女</span>
							<span v-else = "item.sex === 1">男</span>
							</view>
						</u-col>
						<u-col span="12">
...
<script>
import { getStudentPreviewPage,getGradeList } from "@/static/api/principal/principal";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      // 执行查询
      getStudentPreviewPage(params).then(response => {
