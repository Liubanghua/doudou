		...
		<view v-if="current === 0">
			...
			<u-card class="task-list-item" :border="false" padding="20" margin="20rpx" @click="navTo('/pages/sys/workbench/student-info',item.deptId)">
				...
				<view class="" slot="body">
					<u-row gutter="16">
						<u-col span="6">
							<view class="apply-text"><span>年级人数：</span>{{item.total}}</view>
						</u-col>
						<u-col span="6">
...
<script>
import { getGradePage,getGradeList } from "@/static/api/principal/principal";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      // 执行查询
      getGradePage(params).then(response => {
	...
	navTo(url,deptId) {
		uni.navigateTo({
			url: url + '?flag=grade&deptId=' + deptId,
		});
	},
    /** 搜索按钮操作 */
    handleQuery() {
