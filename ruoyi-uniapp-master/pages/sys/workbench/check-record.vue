		...
		<view v-if="current === 0">
			...
			<u-card class="task-list-item" :border="false" padding="20" margin="20rpx">
				<view slot="head" style="display: flex;align-items: center;justify-content: space-between;">
					<view style="color: #999999;font-size: 28rpx;"><span>时间：</span>{{item.createTime}}</view>
				</view>
				<view class="" slot="body">
					<u-row gutter="16">
						<u-col span="12">
							<view class="apply-text"><span>等级：</span>
							<span v-if = "item.gradeName === 1">很差</span>
							<span v-else-if = "item.gradeName === 2">差</span>
							<span v-else-if = "item.gradeName === 3">一般</span>
							<span v-else-if = "item.gradeName === 4">好</span>
							<span v-else = "item.gradeName === 5">优秀</span>
							</view>
						</u-col>
						<u-col span="12">
							<view class="apply-text"><span>备注：</span>{{item.remarks}}</view>
						</u-col>
					</u-row>
				</view>
			</u-card>
...
<script>
import { getInspectPage,getGradeList } from "@/static/api/principal/principal";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      // 执行查询
      getInspectPage(params).then(response => {
	...
	search(value) {
		...
		//先清空展示的数据
			...
			for (var i = 0; i < totalList.length; i++) {
				...
				//（核心所在，其它都是根据需求来自己写）
				if (totalList[i].createTime.indexOf(value) >= 0) {
					displayList.push(totalList[i])
				}else if (totalList[i].remarks.indexOf(value) >= 0) {
					displayList.push(totalList[i])
