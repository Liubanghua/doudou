<template>
	<view class="wrap" style="padding-bottom: 60px;">
		<view v-if="current === 0">
			...
			<u-card class="task-list-item" :border="false" padding="20" margin="20rpx" @click="navTo('/pages/sys/book/personal-details')">
				<view slot="head" style="display: flex;align-items: center;justify-content: space-between;">
					...
					<view style="color: #999999;font-size: 22rpx;"><span>密接时间：</span>{{item.createTime}}</view>
				</view>
				<view class="" slot="body">
					<u-row gutter="16">
						<u-col span="12">
							<view class="apply-text"><span>年级：</span>{{item.gradeName}}</view>
						</u-col>
						<u-col span="12">
							<view class="apply-text"><span>班级：</span>{{item.className}}</view>
						</u-col>
						<u-col span="12">
							<view class="apply-text"><span>学号：</span>{{item.studentNo}}</view>
						</u-col>
					</u-row>
				</view>
				<view class="apply-list-foot" slot="head" style="text-align: right;color: #f28c03;">
					<div v-if="item.grade === 1">
						<u-tag text="密接" type="error"></u-tag>
					</div>
					<div v-if="item.grade === 2">
						<u-tag text="次密接" type="warning"></u-tag>
					</div>
				</view>
			</u-card>
			</view>
...
<script>
import { getTouchPage,getGradeList } from "@/static/api/principal/principal";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      // 执行查询
      getTouchPage(params).then(response => {
	...
	search(value) {
		...
		//先清空展示的数据
			...
			for (var i = 0; i < totalList.length; i++) {
				...
				//（核心所在，其它都是根据需求来自己写）
				if (totalList[i].gradeName.indexOf(value) >= 0) {
					displayList.push(totalList[i])
				}else if (totalList[i].grade == value) {
					displayList.push(totalList[i])
				}else if (totalList[i].className.indexOf(value) >= 0) {
					displayList.push(totalList[i])
