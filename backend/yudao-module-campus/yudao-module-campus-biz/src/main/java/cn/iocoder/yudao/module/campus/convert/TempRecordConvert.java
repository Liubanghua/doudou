...
public interface TempRecordConvert {
    ...
    default List<TempRecordSchoolPreviewRespVO> convertSchool(List<TempRecordSchoolPreviewRespVO> vos, Map<Long, DeptDO> deptMap) {
        List<TempRecordSchoolPreviewRespVO> respVOS = new ArrayList<>();
        Map<Long, TempRecordSchoolPreviewRespVO> vosMap = vos.stream().collect(Collectors.toMap(TempRecordSchoolPreviewRespVO::getDeptId, resp -> resp));
        for (Map.Entry<Long, DeptDO> entry : deptMap.entrySet()) {
            TempRecordSchoolPreviewRespVO vo = vosMap.get(entry.getKey());
            if (vo == null) {
                vo = new TempRecordSchoolPreviewRespVO();
                vo.setDeptId(entry.getKey());
                vo.setTotal(0);
                vo.setFever(0);
                vo.setNormal(0);
            ...
            vo.setSchoolName(entry.getValue().getName());
            respVOS.add(vo);
        }
        return respVOS;
    }
