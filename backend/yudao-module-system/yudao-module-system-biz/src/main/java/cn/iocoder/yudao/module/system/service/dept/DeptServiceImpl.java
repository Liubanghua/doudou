...
public class DeptServiceImpl implements DeptService {
    ...
    @Override
    public Map<Long, DeptDO> getStudentDeptMapByIds(Collection<Long> deptIds) {
        if (CollUtil.isEmpty(deptIds)) {
            return null;
        }

        List<DeptDO> classDepts = this.getDepts(deptIds);
        Map<Long, DeptDO> classDeptMap = CollectionUtils.convertMap(classDepts, DeptDO::getId);

        if (CollUtil.isEmpty(classDeptMap)) {
            return null;
        }

        Collection<Long> gradeDeptIds = CollectionUtils.convertList(classDepts, DeptDO::getParentId);
        List<DeptDO> gradeDepts = this.getDepts(gradeDeptIds);
        Map<Long, DeptDO> gradeDeptMap = CollectionUtils.convertMap(gradeDepts, DeptDO::getId);
        gradeDeptMap.putAll(classDeptMap);
        return gradeDeptMap;
    }

    @Override
    public List<Long> getDeptsByParents(List<Long> parentIds) {
        if (CollUtil.isEmpty(parentIds)) {
            return null;
        }
        QueryWrapperX queryWrapper = new QueryWrapperX();
        queryWrapper.inIfPresent("parent_id", parentIds);
        return deptMapper.getGradeDeptIdsByParentIds(parentIds);
    }
}
